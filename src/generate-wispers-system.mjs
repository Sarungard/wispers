...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'wispers', 'Wispers', and 'WISPERS'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.wispers/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.wispers/g),
   ...
   * Rename files.
   *
   * Rename files that had wispers in their name, such as
   * css/wispers.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*wispers*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('wispers', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-wispers-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Wispers's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace wispers name mentions in files.
      generator.replaceFileContents();
      // Rename files that had wispers in their name.
