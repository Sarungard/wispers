...
There are **two `Hooks.once("ready", ...)` registrations** in `wispers.js` — one for GM-only init logic, one that attaches both the `hotbarDrop` listener **and** the `combatTurnChange` initiative re-prompt hook (see `modules/combat/CLAUDE.md`). Both run; this is intentional, not a duplicate.
